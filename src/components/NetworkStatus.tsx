...
export default function NetworkStatus() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-red-500 to-red-600 text-white text-center py-3 text-sm font-medium animate-slide-down shadow-lg">
      <div className="flex items-center justify-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold">⚠️ 离线模式</span>
        </div>
        <span className="hidden sm:inline text-white/90">|</span>
        <span className="hidden sm:inline text-white/90">部分功能可能不可用，请检查网络连接</span>
      </div>
    </div>
  );
}
